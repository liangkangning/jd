...
<?=$form->field($model, 'np')->checkboxList(['h'=>'头条[h]','c'=>'推荐[c]','f'=>'幻灯[f]','a'=>'特荐[a]','p'=>'图片[p]'],['value'=>explode(',',$model->np)])->label('自定义类型');?>
...
<?=$form->field($model, 'canshu')->textarea(['class'=>'form-control c-md-4', 'rows'=>5])->label('部分参数') ?>

<?=$form->field($model, 'zonghui')->widget('\kucha\ueditor\UEditor',[
    'clientOptions' => [
        'serverUrl' => Url::to(['/public/ueditor']),//确保serverUrl正确指向后端地址
        'lang' =>'zh-cn', //中文为 zh-cn
