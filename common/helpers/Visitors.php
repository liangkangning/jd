...
//        $yisou = count(explode('一搜',$broswer));
//        if ($yisou>1){
//           $list = Yii::$app->request->get('list');
//           if ($list){
//               $num = explode('-',Yii::$app->request->get('list'));
//               if (count($num)>5){
//                   echo '禁止一搜爬虫';
//                   exit();
//                   return;
...
//
//        }
        $is_zhizhu = count(explode('蜘蛛',$broswer))>1?2:1;//1是个人2是蜘蛛
        $this->is_zhihu = $is_zhizhu;
        $os = $this->get_os();
...
//        获取url的属性个数，用于判断属性组合
        ...
        $visitors->ip = $ip;
        $visitors->froms = $froms;
        $visitors->add_time = time();
        $visitors->system = $os;
        $visitors->browser = $broswer;
...
//获取操作系统信息
    ...
    function GetIpFrom($ip = ''){
        ...
        //var_dump($json);
        $address[0] = $res?$json['data']['country'].$json['data']['region'].$json['data']['city'].$json['data']['isp']:'';
        $address[1] = $ip;
