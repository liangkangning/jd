...

{
    ...
    public function actionNewsList(){
        ...
        $ids = Article::find()->where(['status' => 1])->select('id')->orderBy('id asc')->limit('49990')->asArray()->all();
    ...
    public function actionNewsList1(){
        ...
        $ids = Article::find()->where(['status' => 1])->select('id')->orderBy('id asc')->offset('49990')->limit('50000')->asArray()->all();
        $ids = array_column($ids, 'id');
        foreach ($ids as $id) {
            $this->data[]=[
                'loc' => $this->home.'/news/'.$id.'.html',
    ...
    public function actionNewsList2(){
        ...
        $ids = Article::find()->where(['status' => 1])->select('id')->orderBy('id asc')->offset('99990')->limit('50000')->asArray()->all();
        $ids = array_column($ids, 'id');
        foreach ($ids as $id) {
            $this->data[]=[
                'loc' => $this->home.'/news/'.$id.'.html',
