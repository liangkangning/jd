...
//        var_dump($tuijian);
        ...
        if(strpos($url,'mip') !==false){
            ...
            return $this->render('index_mip',['data'=>$this->data]);
        }else{
...
//            获取所有产品的id
            ...
            $description = '钜大锂电,16年'.$keyw['keyword'].'生产厂家,按需定制'.$keyw['keyword'].'方案,价格,品牌.「60余研发团队,3000成功案例」东莞'.$keyw['keyword'].'厂家,国家高新技术,军工资质企业,锂电池热线:400-666-3615';
...
//        相关新闻
            ...
            Yii::$app->params['new_news']=Article::find()->where(['category_id'=>[36,37,38]])->andWhere(['status'=>1])->orderBy(['create_time'=>SORT_DESC])->limit(10)->all();
            $keywords_category=Keywords::find()->orderBy(['sort'=>SORT_ASC])->limit(24)->asArray()->all();
//        var_dump($keywords_category);
            ...
            if(strpos($url,'mip') !==false){
                $this->layout="main_list_mip";
                return $this->render('item_mip',['data'=>$this->data]);
            }else{
